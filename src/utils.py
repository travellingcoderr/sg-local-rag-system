...
def chunk_text_by_characters(
    text: str, chunk_size: int, overlap: int = 100
) -> List[str]:
    """
    Splits text into chunks by character count with overlap (for RAG indexing).
    Use this when TEXT_CHUNK_SIZE is in characters (e.g. 300).
    ...
    logging.info(
        f"Text split into {len(chunks)} character-based chunks (size={chunk_size}, overlap={overlap})."
    )
    return chunks
